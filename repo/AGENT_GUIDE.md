...
Set `CHANNEL_URL` (for example, `https://farcaster.xyz/~/channel/openclaw`) and post with the same CLI command:

```bash
PRIVATE_KEY=0x... SIGNER_PRIVATE_KEY=... FID=123 CHANNEL_URL="https://farcaster.xyz/~/channel/openclaw" node src/post-cast.js "Hello from a channel"
```

The SDK sets `castBody.parentUrl` when `channelUrl` is provided so the cast is routed to channel context.

...
description: Create Farcaster accounts and post, reply, mention, and delete casts autonomously (with explicit channel posting support).
...
To post a cast inside a Farcaster channel, set `CHANNEL_URL` in env. Use a valid Farcaster channel URL (example: `https://farcaster.xyz/~/channel/openclaw`) and post normally as a root cast.

```bash
cd {baseDir}/..
PRIVATE_KEY=0x... SIGNER_PRIVATE_KEY=... FID=123 CHANNEL_URL="https://farcaster.xyz/~/channel/openclaw" node src/post-cast.js "Hey openclaw" 
```

The cast body uses `parentUrl` under the hood, which is Farcaster's channel routing field.
...
If you only have the parent cast hash (and not the author FID), look it up first and then reply. In OpenClaw you can also use the `neynar` skill to resolve cast hash -> `author.fid`.

```javascript
const parentHash = '0xabc123...';
...
await postCast({
  ...
  fid: Number(creds.fid),
  text: 'Great point!',
  parent: { fid: parentFid, hash: parentHash }
});
```
...
If you have a username/handle but not an FID, resolve it first (Neynar can look up user -> `fid`). If the mention isn't at position 0, compute the byte offset:

```javascript
function byteOffset(str, charIndex) {
  return Buffer.from(str.slice(0, charIndex)).length;
}

const text = 'Hey  check this out'; // placeholder space at char index 4
await postCast({
  privateKey: creds.custodyPrivateKey,
  signerPrivateKey: creds.signerPrivateKey,
  fid: Number(creds.fid),
  text,
  mentions: [{ fid: 194, position: byteOffset(text, 4) }]
});
```
